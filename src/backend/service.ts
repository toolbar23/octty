...
export class WorkspaceService {
  ...
  private readonly agentAttentionScreens = new Map<string, string>();
  ...
  constructor(private readonly dbPath = resolveStateDbPath()) {
    ...
    this.ptySidecar.onMessage((message) => {
      if (message.type === "output" && message.sessionId && message.data) {
        ...
        if (session && isAgentTerminalKind(session.kind) && session.state === "live") {
  ...
  dispose(): void {
    ...
    this.agentAttentionScreens.clear();
  ...
  async syncProjectRoot(rootId: string, rootPath: string): Promise<void> {
    ...
    for (const workspace of discovered) {
      ...
      const summary: WorkspaceSummary = {
        ...
        hasConflicts: existing?.hasConflicts ?? false,
        unpublishedChangeCount: existing?.unpublishedChangeCount ?? 0,
        unpublishedAddedLines: existing?.unpublishedAddedLines ?? 0,
        unpublishedRemovedLines: existing?.unpublishedRemovedLines ?? 0,
        notInDefaultAvailable: existing?.notInDefaultAvailable ?? false,
        notInDefaultChangeCount: existing?.notInDefaultChangeCount ?? 0,
        notInDefaultAddedLines: existing?.notInDefaultAddedLines ?? 0,
        notInDefaultRemovedLines: existing?.notInDefaultRemovedLines ?? 0,
  ...
  private scheduleAgentAttentionSettle(sessionId: string): void {
    if (this.attentionTimers.has(sessionId)) {
      return;
    }
    if (!this.agentAttentionScreens.has(sessionId)) {
      this.agentAttentionScreens.set(sessionId, this.ptySidecar.captureScreen(sessionId));
    }
    const timer = setTimeout(() => {
      this.attentionTimers.delete(sessionId);
      const session = this.ptySidecar.getSession(sessionId);
      if (!session || session.state !== "live" || !isAgentTerminalKind(session.kind)) {
        this.agentAttentionScreens.delete(sessionId);
        return;
      }

      const previousScreen = this.agentAttentionScreens.get(sessionId) ?? "";
      const currentScreen = this.ptySidecar.captureScreen(sessionId);
      this.agentAttentionScreens.set(sessionId, currentScreen);

      // Codex can repaint an unchanged waiting prompt continuously, so settle on
      // stable captured screen content rather than raw PTY byte silence.
  ...
  private async refreshWorkspace(workspaceId: string): Promise<void> {
    ...
    this.db.updateWorkspaceStatus(workspaceId, {
      ...
      unpublishedChangeCount: status.unpublishedChangeCount,
      unpublishedAddedLines: status.unpublishedAddedLines,
      unpublishedRemovedLines: status.unpublishedRemovedLines,
      notInDefaultAvailable: status.notInDefaultAvailable,
      notInDefaultChangeCount: status.notInDefaultChangeCount,
      notInDefaultAddedLines: status.notInDefaultAddedLines,
      notInDefaultRemovedLines: status.notInDefaultRemovedLines,
  ...
  setSessionFocused(sessionId: string, focused: boolean): void {
    ...
    if (focused) {
      ...
      } else if (session.agentAttentionState === "thinking") {
        this.scheduleAgentAttentionSettle(sessionId);
      }
      return;
    }
